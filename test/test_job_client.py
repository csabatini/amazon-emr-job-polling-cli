...
def test_adds_expected_python_job(aws_api, grequests):
    expected_output = \
        """
    ...
    output = emr.job_client.handle_job_request(params, aws_api)
    assert output
    assert textwrap.dedent(expected_output).replace('\n', '') == output


def test_shutdown_false_no_shutdown_api_calls(aws_api, grequests):
    test_params = copy.deepcopy(params)
    test_params['job_runtime'] = 'Scala'
    emr.job_client.handle_job_request(test_params, aws_api)
...
def test_shutdown_true_makes_shutdown_api_calls(aws_api, grequests):
    test_params = copy.deepcopy(params)
    test_params['job_runtime'] = 'Scala'
...
def test_cluster_job_not_added_conditions(aws_api, grequests):
    test_params = copy.deepcopy(params)
